...
static const char* streamType = "multipart/x-mixed-replace;boundary=" BOUNDARY_STR;
static const char* boundary = "\r\n--" BOUNDARY_STR "\r\n";
static const char* headerFmt = "Content-Type: image/jpeg\r\nContent-Length: %u\r\n\r\n";
  ...
  while (true) {
    ...
    if (err == ESP_OK) {
      err = httpd_resp_send_chunk(
        req, boundary, strlen(boundary)
      );
    }

    if (err == ESP_OK) {
      size_t headerLen = snprintf(
        header, sizeof(header), headerFmt, dataLen
      );
      err = httpd_resp_send_chunk(
        req, header, headerLen
      );
    }

    if (err == ESP_OK) {
      err = httpd_resp_send_chunk(
        req, (const char*)data, dataLen
      );
    }
